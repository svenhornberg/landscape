---
geschaeftsfeld: gk
---

# Geschaeftskunden

Stand der Interviews: September 2026.

| Abteilung   | Aufgabe             | System | Anmerkung                          |
|-------------|---------------------|--------|------------------------------------|
| vertrieb    | angebot.erstellen   | crm    | Standardweg seit 2021              |
| vertrieb    | angebot.erstellen   | excel  | Sonderkalkulation bei Rahmenpreisen|
| vertrieb    | angebot.versenden   | crm    |                                    |
| vertrieb    | angebot.nachfassen  | crm    |                                    |
| vertrieb    | bestellung.erfassen | erp    |                                    |
| vertrieb    | kunde.anlegen       | crm    |                                    |
| service     | bestellung.erfassen | ticket | telefonisch aufgenommen            |
| service     | bestellung.pruefen  | erp    |                                    |
| service     | kunde.anlegen       | crm    |                                    |
| service     | retoure.erfassen    | ticket |                                    |
| lager       | bestellung.versenden| erp    |                                    |
| lager       | retoure.erfassen    | erp    |                                    |
| lager       | retoure.erstatten   | erp    |                                    |
| buchhaltung | rechnung.erstellen  | erp    |                                    |
| buchhaltung | rechnung.mahnen     | mahn   |                                    |
| buchhaltung | rechnung.mahnen     | excel  | manuelle Liste fuer Sonderfaelle   |
| buchhaltung | kunde.bonitaet      | erp    |                                    |
